...
RU: Final[dict[str, str]] = {
    ...
    "challenge.button": "🛡 Открыть браузер",
    "challenge.retry_button": "✅ Проверку прошёл — повторить",
    "challenge.retrying": "Повторяю запрос…",
    "challenge.retry_expired": "Ссылка на повтор устарела. Повторите действие заново.",
    "challenge.admin_prompt": (
        "rutracker закрыт проверкой Cloudflare. Откройте ссылку, поставьте "
        "галочку «Я человек», вернитесь сюда и нажмите «Проверку прошёл». "
        "Ссылка действует 30 минут."
    ),
    "challenge.login_prompt": (
        "Сессия rutracker разлогинена. Откройте ссылку, при необходимости "
        "пройдите проверку Cloudflare, войдите логином и паролем, вернитесь "
        "сюда и нажмите «Проверку прошёл». Ссылка действует 30 минут."
    ),
    "challenge.user_wait": (
        "rutracker закрыт проверкой Cloudflare — администратор уже получил "
        "ссылку для её прохождения. Попробуйте через пару минут."
    ),
    "challenge.login_user_wait": (
        "Сессия rutracker разлогинена — администратор уже получил ссылку для "
        "входа. Попробуйте через пару минут."
    ),
    "challenge.admin_pinged": (
        "Пользователь наткнулся на проверку Cloudflare на rutracker. "
        "Пройдите её по кнопке и нажмите «Проверку прошёл» — запрос "
        "повторится здесь, в этом чате."
    ),
    "challenge.login_admin_pinged": (
        "Пользователь упёрся в разлогиненную сессию rutracker. Войдите по "
        "кнопке и нажмите «Проверку прошёл» — запрос повторится здесь, в "
        "этом чате."
    ),
    ...
    "ydl.youtube_blocked": (
        "Ссылка YouTube распознана, но YouTube отклонил запрос сервера "
        "(проверка «я не робот»). Скорее всего, на сервере загрузок устарели "
        "cookies YouTube — нужно вмешательство администратора."
    ),
